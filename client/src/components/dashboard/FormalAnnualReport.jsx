    ...
    return (
        ...
        >
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
                ...
                {/* Entity Information */}
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '48px', marginBottom: '36px', fontSize: '13px' }}>
                    <div>
                ...
                <div style={{ marginBottom: '40px' }}>
                    <h3 style={{ fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '2px', background: '#f3f4f6', padding: '8px 12px', borderLeft: '4px solid #111', marginBottom: '16px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', border: '1px solid #ddd', textAlign: 'center' }}>
                        <div style={{ padding: '16px', borderRight: '1px solid #ddd' }}>
                            ...
                            <p style={{ fontSize: '18px', fontWeight: 900, color: '#047857', margin: 0 }}>{formatCurrency(totalRevenue)}</p>
                        </div>
                        <div style={{ padding: '16px', borderRight: '1px solid #ddd' }}>
                            <p style={{ fontSize: '9px', textTransform: 'uppercase', fontWeight: 700, color: '#888', margin: '0 0 4px 0' }}>Dépenses Totales</p>
                            <p style={{ fontSize: '18px', fontWeight: 900, color: '#be123c', margin: 0 }}>{formatCurrency(totalExpense)}</p>
                        </div>
                        <div style={{ padding: '16px', background: '#f9fafb' }}>
                            <p style={{ fontSize: '9px', textTransform: 'uppercase', fontWeight: 700, color: '#888', margin: '0 0 4px 0' }}>Bilan Net Annuel</p>
                            <p style={{ fontSize: '18px', fontWeight: 900, textDecoration: 'underline', color: netResult >= 0 ? '#111' : '#9f1239', margin: 0 }}>{formatCurrency(netResult)}</p>
                        </div>
                    </div>
                </div>
                ...
                <div style={{ marginBottom: '32px' }}>
                    <h4 style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '2px', color: '#047857', marginBottom: '12px' }}>1. Détail des Encaissements</h4>
                    <table style={{ width: '100%', fontSize: '11px', borderCollapse: 'collapse', border: '1px solid #ddd' }}>
                        <thead>
                            <tr style={{ background: '#f9fafb', borderBottom: '1px solid #ddd' }}>
                                <th style={{ padding: '8px', textAlign: 'left', borderRight: '1px solid #ddd', textTransform: 'uppercase', fontSize: '10px' }}>Désignation</th>
                                <th style={{ padding: '8px', textAlign: 'right', textTransform: 'uppercase', fontSize: '10px' }}>Montant Consolidé</th>
                            </tr>
                        </thead>
                        <tbody>
                            {revenueCategories.map(cat => (
                                <tr key={cat.category} style={{ borderBottom: '1px solid #eee' }}>
                                    <td style={{ padding: '8px', borderRight: '1px solid #ddd' }}>{cat.label}</td>
                                    <td style={{ padding: '8px', textAlign: 'right', fontWeight: 700 }}>{cat.total.toLocaleString()} €</td>
                                </tr>
                            ))}
                            {revenueCategories.length === 0 && (
                                <tr><td colSpan="2" style={{ padding: '12px', textAlign: 'center', fontStyle: 'italic', color: '#999' }}>Aucun encaissement enregistré</td></tr>
                            )}
                            <tr style={{ background: '#f9fafb', fontWeight: 900, fontStyle: 'italic' }}>
                                <td style={{ padding: '8px', textAlign: 'right', borderRight: '1px solid #ddd' }}>SOUS-TOTAL RECETTES</td>
                                <td style={{ padding: '8px', textAlign: 'right', color: '#047857', textDecoration: 'underline' }}>{totalRevenue.toLocaleString()} €</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                {/* Expense Table */}
                <div style={{ marginBottom: '40px' }}>
                    <h4 style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '2px', color: '#be123c', marginBottom: '12px' }}>2. Détail des Décaissements</h4>
                    <table style={{ width: '100%', fontSize: '11px', borderCollapse: 'collapse', border: '1px solid #ddd' }}>
                        <thead>
                            <tr style={{ background: '#f9fafb', borderBottom: '1px solid #ddd' }}>
                                <th style={{ padding: '8px', textAlign: 'left', borderRight: '1px solid #ddd', textTransform: 'uppercase', fontSize: '10px' }}>Désignation</th>
                        ...
                        <tbody>
                            ...
                            {expenseCategories.length === 0 && (
                                <tr><td colSpan="2" style={{ padding: '12px', textAlign: 'center', fontStyle: 'italic', color: '#999' }}>Aucune charge enregistrée</td></tr>
                            )}
                            <tr style={{ background: '#f9fafb', fontWeight: 900, fontStyle: 'italic' }}>
                                <td style={{ padding: '8px', textAlign: 'right', borderRight: '1px solid #ddd' }}>SOUS-TOTAL DÉPENSES</td>
                                <td style={{ padding: '8px', textAlign: 'right', color: '#be123c', textDecoration: 'underline' }}>{totalExpense.toLocaleString()} €</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                {/* Footer */}
                <div style={{ borderTop: '1px solid #ddd', paddingTop: '32px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px', fontSize: '9px', lineHeight: 1.6, fontStyle: 'italic', color: '#888' }}>
                    <div>
